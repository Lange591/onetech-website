...
function SocialButtons() { const items=[{icon:Facebook,label:"Facebook",href:socialLinks.facebook},{icon:Instagram,label:"Instagram",href:socialLinks.instagram},{icon:Linkedin,label:"LinkedIn",href:socialLinks.linkedin},{icon:Globe,label:"Website",href:socialLinks.website}]; return <div className="mt-8 flex flex-wrap gap-3">{items.map(i=><a key={i.label} href={i.href} target="_blank" className="inline-flex items-center gap-2 rounded-full bg-red-600 px-5 py-3 font-black text-white"><i.icon size={18}/>{i.label}</a>)}</div> }
