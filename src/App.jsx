...
import {
  ...
  Facebook, Instagram, Linkedin, MessageCircleMore, Globe2, UsersRound
} from "lucide-react";
...
const socialLinks = {
  facebook: "#",   // replace with OneTech Facebook link
  instagram: "#",  // replace with OneTech Instagram link
  linkedin: "#",   // replace with OneTech LinkedIn link
  website: "#",    // replace with company website/custom domain
};
...
function WhatsAppFloat() {
  return <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hi OneTech, I would like to request a quotation.")}`} target="_blank" className="fixed bottom-6 right-6 z-[90] flex items-center gap-3 rounded-full bg-green-500 px-5 py-4 font-black text-white shadow-2xl shadow-green-900/40 transition hover:scale-105"><MessageCircleMore /> Chat on WhatsApp</a>;
}
...
function SocialButtons() { const items=[{icon:Facebook,label:"Facebook",href:socialLinks.facebook},{icon:Instagram,label:"Instagram",href:socialLinks.instagram},{icon:Linkedin,label:"LinkedIn",href:socialLinks.linkedin},{icon:Globe2,label:"Website",href:socialLinks.website}]; return <div className="mt-8 flex flex-wrap gap-3">{items.map(i=><a key={i.label} href={i.href} target="_blank" className="inline-flex items-center gap-2 rounded-full bg-red-600 px-5 py-3 font-black text-white"><i.icon size={18}/>{i.label}</a>)}</div> }
