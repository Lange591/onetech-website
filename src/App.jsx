...
const teamMembers = [
  ...
  },
  {
    ...
    img: "/images/team/nixon-kunze-studio.jpg",
    objPos: "50% 15%",
...
function Logo({ small = false }) {
  return (
    <span
      className={`inline-flex items-center justify-center rounded-2xl bg-gradient-to-b from-white to-zinc-100 ring-1 ring-white/60 shadow-[0_6px_24px_-6px_rgba(220,38,38,0.35),0_2px_8px_-2px_rgba(0,0,0,0.6)] ${
        small ? "h-12 px-3.5 sm:h-14 sm:px-4" : "h-[4.5rem] px-5 sm:h-[5.25rem] sm:px-6"
      }`}
    >
      <img
        src="/images/brand/logo-transparent.png"
        alt="OneTech IT Services"
        className={`${small ? "h-8 sm:h-9" : "h-12 sm:h-14"} w-auto object-contain`}
      />
    </span>
  );
}
  ...
  return (
    <div className="relative mx-auto h-[650px] w-full max-w-[650px]">
      ...
      <div className="absolute left-1/2 top-1/2 h-[330px] w-[330px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[3.5rem] border border-white/15 bg-white/10 p-5 shadow-2xl backdrop-blur-2xl">
        <img
          src="/images/starlink/real-dish-roof.jpg"
          alt="OneTech technician installing a satellite dish"
          className="h-full w-full rounded-[2.7rem] object-cover opacity-90"
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {flyers.map((flyer, i) => {
            ...
            return (
              ...
              >
                <div className="relative aspect-[3/4] w-full overflow-hidden bg-gradient-to-b from-zinc-900 to-black p-3">
                  <img
                    src={flyer.src}
                    alt={flyer.alt}
                    className="h-full w-full rounded-2xl object-contain transition duration-700 group-hover:scale-[1.03]"
                  />
                </div>
                ...
                <div className="relative p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`rounded-xl bg-gradient-to-br ${flyer.gradient} p-2 shadow-lg ring-1 ring-white/15`}>
                      <Icon className="h-5 w-5 text-white" />
                    </div>
                    <h3 className="text-xl font-black tracking-tight">{flyer.title}</h3>
                  </div>
                  <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-red-500/25 bg-red-500/10 px-3 py-1 text-xs font-bold text-red-300">
            ...
            <div className="grid grid-cols-2 gap-4">
              {service.images.slice(0, 4).map((img, i) => (
                <img
                  ...
                  className={`${i === 0 ? "col-span-2 h-80" : "h-44"} w-full rounded-[2rem] object-cover shadow-2xl`}
                  style={{ objectPosition: "50% 25%" }}
                />
              ))}
            </div>
        ...
        >
          <div className="relative aspect-[4/5] overflow-hidden">
            <img
              ...
              alt={t.name}
              className="h-full w-full object-cover"
              style={{ objectPosition: t.objPos || "50% 15%" }}
            />
